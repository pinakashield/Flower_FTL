...
num_features = pretrain_info.get("num_features")  # Fix applied: Use num_features instead of server_features
...
try:
    server_model.load_weights("pretrained_global_model.weights.h5")  # Ensure correct file extension
...
class FTLStrategy(fl.server.strategy.FedAvg):
    def aggregate_fit(self, server_round, results, failures):
        ...
        # Extract and ensure parameters are numeric
        client_weights = [np.array(res.parameters.tensors, dtype=np.float32) for _, res in results]
        ...
        return fl.common.Weights(aggregated_weights), {}
